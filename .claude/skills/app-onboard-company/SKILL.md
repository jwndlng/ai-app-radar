# Keep in sync with .gemini/skills/app-onboard-company/SKILL.md

---
name: app-onboard-company
description: Use this skill when the user wants to add a new company to the scout, classify its ATS, verify the config is live, and append a correctly structured entry to configs/companies.json.
---
...
For Workday, the `careers_url` is used directly — no slug config needed. If the URL contains a locale segment like `/en-US/`, use the canonical URL without the locale prefix.
...
- `workday_api` — extract tenant and board from the URL (tenant = first subdomain, board = first non-locale path segment), then: `curl -s -o /dev/null -w "%{http_code}" -X POST -H "Content-Type: application/json" -d '{"appliedFacets":{},"limit":1,"offset":0,"searchText":""}' "https://{tenant}.wd1.myworkdayjobs.com/wday/cxs/{tenant}/{board}/jobs"`
...
**Description**: Write a concise 1-sentence description of what the company does. Show it and ask the user to confirm, edit, or skip (leave field absent).

**Category**: Infer the most appropriate `category` tag (e.g. `"AI/ML"`, `"Fintech"`, `"MedTech"`, `"Robotics"`, `"Cloud"`, `"Enterprise"`, `"CleanTech"`, `"Blockchain"`, `"Social"`). Show the inferred category and ask the user to confirm, override, or skip.
...
**For known ATS (e.g. Ashby, Lever, Workable):**
```json
{
  "name": "<Company Name>",
