...
- **WHEN** `run()` is called
- **THEN** the count of jobs with status `"enriched"` SHALL be computed upfront and passed to `log.start(total=N)` before the evaluation loop begins

#### Scenario: No enriched jobs produces a warning and exits
- **WHEN** no jobs have status `"enriched"`
