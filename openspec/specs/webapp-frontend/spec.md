...
Clicking a job card SHALL expand an inline detail panel showing all available job fields (title, company, location, score, matched skills, pros/cons, summary, links). The detail panel SHALL also display the same state-aware action buttons as the card view.
