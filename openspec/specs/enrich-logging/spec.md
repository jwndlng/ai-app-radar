...
- **WHEN** there are no jobs with status `"new"`
...
- **WHEN** `EnrichAgent.extract()` returns valid data and the job status is set to `"enriched"`
- **THEN** `log.item_ok(name, label="enrich", detail="enriched", elapsed=...)` SHALL be called
