...
Specification for archiving terminal-state jobs (rejected, auto-rejected, and old failed jobs) out of the active `applications.json` store into a separate archive file, keeping the active store small while preserving history.
